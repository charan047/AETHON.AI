...
test.describe('Company Chat', () => {
  test('shows the redesigned empty state and sidebar collapse affordance', async ({ page, request }) => {
    ...
    await expect(page.getByText(/Research this week's competitor moves/i)).toBeVisible()
    await expect(page.getByRole('button', { name: 'Show me company analytics' }).first()).toBeVisible()
    await expect(page.getByText(/Do I have anything waiting for approval/i)).toBeVisible()
