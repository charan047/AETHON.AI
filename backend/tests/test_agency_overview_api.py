...
async def test_agency_overview_returns_sorted_attention_items(
    ...
    db.add_all([pending_review_execution, failed_workflow])
    await db.commit()
    ...
    review_item = payload["needs_attention"][1]
    assert review_item["title"] == f"Review ready: {test_workflow.name}"
    assert review_item["client_name"] == "Acme Corp"
    assert review_item["execution_id"] == pending_review_execution.id
    ...
    failed_item = payload["needs_attention"][2]
